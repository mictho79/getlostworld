...
async function loadCountries() {
  ...
  console.log('Total layers:', geoLayer.getLayers().length);
  geoLayer.getLayers()[10].on('mouseover', () => console.log('RAW MOUSEOVER WORKS'));
...
async function boot() {
  ...
  try {
    await loadCountries();
    console.log('GeoJSON layers loaded:', geoLayer?.getLayers()?.length);
  } catch (err) {
